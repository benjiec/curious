...
function curiousJoinTable(results,set_table_cb,object_cache_f,get_objects_f){
 ...
 var GET_BATCH=500; var DEFAULT_PAGE_SIZE=100;var entries=[];var models=[]; var tbl_queries=[];var tbl_attrs=[];var tbl_rows=[];var tbl_view=undefined;var tbl_csv=undefined;var tbl_controls={};var pourover_collection=undefined;var pourover_sorters=undefined;var outstanding_fetches=0;
...
function get_objects(model,ids,cb){var cb_data=undefined;var unfetched=[];for(var i=0;i<ids.length;i++){var id=ids[i];var obj_id=model+'.'+id;if(obj_id in object_cache_f()&&object_cache_f()[obj_id]['__fetched__']){cb_data=object_cache_f()[obj_id]['__fetched__'];}
else{unfetched.push(id);}}
if(cb_data!==undefined&&cb){cb(cb_data);}
if(unfetched.length>0){while(unfetched.length>0){var tofetch=unfetched.slice(0,GET_BATCH);var unfetched=unfetched.slice(GET_BATCH);outstanding_fetches+=1;get_objects_f(model,tofetch,function(results){outstanding_fetches-=1;for(var i=0;i<results.length;i++){var obj_data=results[i];add_object_data(model,obj_data);if(cb_data===undefined&&cb){cb_data=obj_data;cb(obj_data);}}});}}}
...
var sorters=_create_sorter(column_index);var sorter;if(tbl_controls.sort_column===column_index){if(tbl_controls.sort_asc===false){tbl_controls.sort_asc=true;sorter=sorters[0];}
