...
$scope.checkQuery=function(cb){$scope._check_query($scope.query,function(query_string,err){$scope.query_error=err;$scope.query_accepted=query_string;if(cb!==undefined){cb(query_string,err);}});};$scope.submitQuery=function(reload){if(reload===undefined){reload=false;}
...
function curiousJoinTable(results,set_table_cb,object_cache_f,get_objects_f){
 ...
 var GET_BATCH=500; var DEFAULT_PAGE_SIZE=100;var entries=[];var models=[]; var tbl_queries=[];var tbl_attrs=[];var tbl_rows=[];var tbl_view=undefined;var tbl_csv=undefined;
...
function execute(){init_query();var query=$scope.query;do_query(query,$scope.query_info.reload,function(result,err){$scope.completed=true;if(err){$scope.success=false;$scope.search_error=err;}
else{$scope.success=true;if(result.length>0){$scope.last_model=result[result.length-1].model;get_model($scope.last_model,function(result,error){if(result){if(result.relationships){$scope.last_model_rels=result.relationships;}}}); curiousJoinTable(result,function(tbl){$scope.table=tbl;},function(){return $scope.$parent._object_cache;},get_objects);}}});}
