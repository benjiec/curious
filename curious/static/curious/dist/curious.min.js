...
function curiousJoinTable(results,set_table_cb,object_cache_f,get_objects_f){
 ...
 var GET_BATCH=500; var DEFAULT_PAGE_SIZE=100;var entries=[];var models=[]; var tbl_queries=[];var tbl_attrs=[];var tbl_rows=[];var tbl_view=undefined;var tbl_csv=undefined;var tbl_controls={};var pourover_collection=undefined;var outstanding_requests=0;
...
for(var i=0;i<entries.length;i++){for(var j=0;j<entries[i].length;j++){var entry=entries[i][j];var obj_id=entry.model+'.'+entry.id;if(object_cache_f()[obj_id]===undefined){var id_str=''+entry.id;if(entry.url){id_str='<a href="'+entry.url+'">'+entry.id+'</a>';}
...
function add_object_data(model,obj_data){var id=obj_data.id;var obj_id=model+'.'+id;var ptr=object_cache_f()[obj_id];ptr['__fetched__']=obj_data;for(var a in obj_data){ if(a!=='id'){
...
function get_objects(model,ids,cb,complete_cb){var cb_data=undefined;var unfetched=[];for(var i=0;i<ids.length;i++){var id=ids[i];var obj_id=model+'.'+id;if(obj_id in object_cache_f()&&object_cache_f()[obj_id]['__fetched__']){cb_data=object_cache_f()[obj_id]['__fetched__'];}
else{unfetched.push(id);}}
if(cb_data!==undefined&&cb){cb(cb_data);}
if(unfetched.length==0&&outstanding_requests==0){complete_cb();}
if(unfetched.length>0){while(unfetched.length>0){var tofetch=unfetched.slice(0,GET_BATCH);var unfetched=unfetched.slice(GET_BATCH);outstanding_requests+=1;get_objects_f(model,tofetch,function(results){outstanding_requests-=1;for(var i=0;i<results.length;i++){var obj_data=results[i];add_object_data(model,obj_data);if(cb_data===undefined&&cb){cb_data=obj_data;cb(obj_data);}}
...
function update_pourover_sorters(){function make_sorter(col,attr,reverse){var ColSorter=PourOver.Sort.extend({attr:'foo',fn:function(a,b){if(a.row[col].ptr[attr]===undefined){return 1;}
if(b.row[col].ptr[attr]===undefined){return-1;}
var v_a=a.row[col].ptr[attr].value;var v_b=b.row[col].ptr[attr].value;if(reverse){var tmp=v_a;v_a=v_b;v_b=tmp;}
if(v_a===undefined||v_a===null){return 1;}
if(v_a>v_b){return-1;}
else if(v_a<v_b){return 1;}
else{return 0;}}});return ColSorter;}
var sorters=[];for(var i=0;i<tbl_attrs.length;i++){var ColSorterAsc=make_sorter(i,tbl_attrs[i].name,true);var ColSorterDsc=make_sorter(i,tbl_attrs[i].name,false);sorters.push(new ColSorterAsc('_asc_col_'+i));sorters.push(new ColSorterDsc('_dsc_col_'+i));}
pourover_collection.addSorts(sorters); tbl_view.setSort('_asc_col_0');tbl_controls.sort_column=0;tbl_controls.sort_asc=true;}
function next_page(){tbl_view.page(1);}
function prev_page(){tbl_view.page(-1);}
function sort(column_index){var sorter_name;if(tbl_controls.sort_column===column_index){if(tbl_controls.sort_asc===false){tbl_controls.sort_asc=true;sorter_name='_asc_col_'+column_index;}
else{tbl_controls.sort_asc=false;sorter_name='_dsc_col_'+column_index;}}
else{tbl_controls.sort_column=column_index;tbl_controls.sort_asc=true;sorter_name='_asc_col_'+column_index;}
...
function update_controller(){ set_table_cb({queries:tbl_queries,attrs:tbl_attrs,length:tbl_rows.length,view:tbl_view,controls:tbl_controls,csv:tbl_csv, toggleQuery:toggle,showCSV:show_csv,nextPage:next_page,prevPage:prev_page,sort:sort});}
...
get_objects(models[query_idx].model,ids,function(data){update_model_attrs(query_idx,data);models[query_idx].loaded=true;},function(){console.log('all fetch completed');update_pourover_sorters();});} 
...
$scope.csv=true;}};$scope.query=$scope.query_info.query;execute();};