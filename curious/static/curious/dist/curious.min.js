...
if(unfetched.length>0){get_objects_f(model,unfetched,function(results){for(var i=0;i<results.length;i++){var id=results[i].id;var obj_data=results[i];var obj_id=model+'.'+id;var ptr=objects[obj_id];ptr['__fetched__']=obj_data;for(var a in obj_data){ if(a!=='id'){
...
if(cb_data===undefined&&cb){cb_data=obj_data;cb(obj_data);}}});}}
...
obj=entries[0][query_idx];get_objects(obj.model,[obj.id],function(data){update_model_attrs(query_idx,data);});return;} 
var ids=[];for(var i=0;i<entries.length;i++){var entry=entries[i][query_idx];ids.push(entry.ptr.id.value);console.log('will fetch '+entry.ptr.id.value);}
