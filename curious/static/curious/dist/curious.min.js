...
function execute(){init_query();var query=$scope.query;console.log(query);do_query(query,function(result,err){$scope.completed=true;if(err){$scope.success=false;$scope.search_error=err;}
else{$scope.success=true;$scope.last_model=result.model;get_model($scope.last_model,function(result,error){if(result){if(result.relationships){$scope.last_model_rels=result.relationships;}}}); curiousJoinTable(result,function(tbl){$scope.table=tbl;},get_object);}});}
