...
if(unfetched.length>0){while(unfetched.length>0){var tofetch=unfetched.slice(0,GET_BATCH);var unfetched=unfetched.slice(GET_BATCH);outstanding_fetches+=1;get_objects_f(model,tofetch,function(results){outstanding_fetches-=1;var fields=results.fields;for(var i=0;i<results.objects.length;i++){var obj_data={};var values=results.objects[i]; obj_data['__url__']=values[0];values=values.slice(1);for(var j=0;j<fields.length;j++){obj_data[fields[j]]=values[j];if(Object.prototype.toString.call(values[j])==='[object Array]'){obj_data[fields[j]]={model:values[j][0],id:values[j][1],__str__:values[j][2],url:values[j][3]}}}
