...
function curiousJoinTable(results,set_table_cb,get_object_f){
 ...
 var entries=[];var objects=[];var models=[]; var tbl_queries=[];var tbl_attrs=[];var tbl_rows=[];var tbl_csv=undefined;
...
for(var j=0;j<entries[0].length;j++){tbl_queries.push({model:entries[0][j].model,cols:1});models.push({model:entries[0][j].model,attrs:['id'],loaded:false});} 
...
else{$scope.success=true;if(result.length>0){$scope.last_model=result[result.length-1].model;get_model($scope.last_model,function(result,error){if(result){if(result.relationships){$scope.last_model_rels=result.relationships;}}}); curiousJoinTable(result,function(tbl){$scope.table=tbl;},get_object);}}});}
