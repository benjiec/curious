...
$scope.delayPromise=$timeout(function(){$scope.checkQuery();$scope.delayPromise=undefined;},1000);};$scope._check_query=function(query_string,cb){ query_string=query_string.replace(/\n/g," ");var url=$scope.__base_url+'/q/';$http.post(url,{c:1,q:query_string}).success(function(data){cb(data.result.query,'');}).error(function(data,status,headers,config){var err='';if(data.error){err=data.error.message;}
...
function add_object_data(model,obj_data){var id=obj_data.id;var obj_id=model+'.'+id;var ptr=object_cache[obj_id];ptr['__fetched__']=obj_data;for(var a in obj_data){ if(a==='id'){var v=obj_data[a];var s=''+v;if(obj_data['__url__']){s='<a href="'+obj_data['__url__']+'">'+v+'</a>';}
...
csv_rows.push(header.join(','));for(var i=0;i<tbl_rows.length;i++){var row=[];for(var j=0;j<tbl_rows[i].length;j++){var entry=tbl_rows[i][j];if(entry){var obj=entry.ptr;if(obj){var v=obj[tbl_attrs[j].name];if(v.display!==undefined){v=v.value;}
if(v!==undefined&&v!==null){v=''+v; v=v.replace("\n"," ");v=v.replace("\t"," ");v=v.replace("\"","");v=v.replace("\'","");v="\""+v+"\"";}
else{v='';}
row.push(v);}
...
create_table(false);}'use strict';function QueryController($scope,$http){function init_query(){$scope.completed=false;$scope.success=false;$scope.search_error=undefined;$scope.last_model=undefined;$scope.last_model_rels=[];$scope.computed_on=undefined;$scope.computed_since=undefined;$scope.table=undefined;$scope.csv=false;}
...
function do_query(query_string,reload,cb){var url=$scope.__base_url+'/q/';var params={q:query_string};if(reload){params['r']=1;}
