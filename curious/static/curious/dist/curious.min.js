...
function curiousJoinTable(results,set_table_cb,object_cache_f,get_objects_f){
 ...
 var GET_BATCH=300; var DEFAULT_PAGE_SIZE=100;var PARTIAL_FETCH=3000; var entries=[];var models=[]; var tbl_queries=[];var tbl_attrs=[];var tbl_rows=[];var tbl_view=undefined;var tbl_csv=undefined;var tbl_controls={};var partial_fetch=undefined;var pourover_collection=undefined;var outstanding_fetches=0; var object_cache=object_cache_f();
 for(var i=0;i<results.length;i++){results[i].map={}
for(var j=0;j<results[i].objects.length;j++){var obj=results[i].objects[j];if(obj[0]!==null){if(results[i].map[obj[2]]===undefined){results[i].map[obj[2]]=[];}
...
var entries=[]; for(var i=0;i<results[0].objects.length;i++){entries.push([build_obj(0,results[0].objects[i])]);} 
for(var col=1;col<results.length;col++){var new_entries=[];var column=results[col];var join_index=column.join_index;for(var i=0;i<entries.length;i++){var row=entries[i];console.log('col '+col+', row '+i+', joins '+join_index)
console.log(row)
var join_pk=row[join_index].id;var objs=column.map[join_pk];if(objs!==undefined){for(var j=0;j<objs.length;j++){var new_row=row;if(objs.length!=1){new_row=row.slice(0);}
new_row.push(build_obj(col,objs[j]));new_entries.push(new_row);console.log('add row')
console.log(new_row)}}
else{row.push(null);new_entries.push(row);console.log('add null')
console.log(row)}}
entries=new_entries;}


for(var i=0;i<entries.length;i++){ for(var j=0;j<entries[i].length;j++){var entry=entries[i][j];if(entry!==null){var obj_id=entry.model+'.'+entry.id;if(object_cache[obj_id]===undefined){var id_str=''+entry.id;if(entry.url){id_str='<a href="'+entry.url+'">'+entry.id+'</a>';}
object_cache[obj_id]={id:{value:entry.id,display:id_str}};}
entry['ptr']=object_cache[obj_id];}}} 
for(var j=0;j<entries[0].length;j++){var model_i=null;for(var i=0;i<entries.length;i++){if(entries[i][j]!==null){model_i=i;break;}}
if(model_i===null){alert('No model for a query. This is really bad!!!');}
tbl_queries.push({model:entries[model_i][j].model,cols:1});models.push({model:entries[model_i][j].model,attrs:[{name:'id',visible:true}],loaded:false});}
...
tbl_view=new PourOver.View('default',pourover_collection,{page_size:page_size});tbl_controls={};tbl_controls.sort_columns=[];tbl_controls.sort_dirs=[];}
...
function update_controller(){ set_table_cb({queries:tbl_queries,attrs:tbl_attrs,length:tbl_rows.length,view:tbl_view,controls:tbl_controls,csv:tbl_csv, toggleQuery:toggle,showCSV:show_csv,nextPage:next_page,prevPage:prev_page,sort:sort,clearSort:clear_sort,makeFilter:make_filter,filter:filter,aggregate:aggregate,partial_fetch:partial_fetch,fetchAll:fetch_all});}
...
return 0;});attrs.unshift(old_attr);models[query_idx].attrs=attrs;tbl_queries[query_idx].cols=attrs.length;update_table();}}
...
update_table(); if(entries.length>PARTIAL_FETCH){partial_fetch=GET_BATCH;} 
if(entries.length>0&&entries[0].length>0){show_object_attrs(entries[0].length-1);}}'use strict';function QueryController($scope,$http){function init_query(){$scope.completed=false;$scope.success=false;$scope.search_error=undefined;$scope.last_model=undefined;$scope.last_model_rels=[];$scope.computed_on=undefined;$scope.computed_since=undefined;$scope.table=undefined;$scope.csv=false;}
