...
for(var j=0;j<results[i].objects.length;j++){var obj=results[i].objects[j];if(results[i].map[obj[2]]===undefined){results[i].map[obj[2]]=[];}
...
for(var col=1;col<results.length;col++){var new_entries=[];var column=results[col];var join_index=column.join_index;for(var i=0;i<entries.length;i++){var row=entries[i];var join_pk=row[join_index].id;var objs=column.map[join_pk];if(objs!==undefined){for(var j=0;j<objs.length;j++){var new_row=row;if(objs.length!=1){new_row=row.slice(0);}
...
$scope.delayPromise=$timeout(function(){$scope.checkQuery();$scope.delayPromise=undefined;},1000);};$scope._check_query=function(query_string,cb){ query_string=query_string.replace(/\n/g," ");var url=$scope.__base_url+'/q/';var url=url+'?c=1&q='+encodeURIComponent(query_string);$http.get(url).success(function(data){cb(data.result.query,'');}).error(function(data,status,headers,config){var err='';if(data.error){err=data.error.message;}
