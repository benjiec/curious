...
var sorters=_create_sorter(column_index);var sorter;if(tbl_controls.sort_column!==undefined&&tbl_controls.sort_column===column_index){if(tbl_controls.sort_asc===false){tbl_controls.sort_asc=true;sorter=sorters[0];}
...
function update_controller(){ set_table_cb({queries:tbl_queries,attrs:tbl_attrs,length:tbl_rows.length,view:tbl_view,controls:tbl_controls,csv:tbl_csv, toggleQuery:toggle,showCSV:show_csv,nextPage:next_page,prevPage:prev_page,sort:sort,make_filter:make_filter,filter:filter,aggregate:aggregate,partial_fetch:partial_fetch,fetchAll:fetch_all});}
...
console.log(attrs);attrs.sort(function(a,b){if(a.name<b.name){return-1;}
else if(a.name>b.name){return 1;}
return 0;});console.log(attrs);attrs.unshift(old_attr);models[query_idx].attrs=attrs;tbl_queries[query_idx].cols=attrs.length;update_table();}}
...
for(var i=0;i<nfetch;i++){var entry=entries[i][query_idx];ids.push(entry.ptr.id.value);}
